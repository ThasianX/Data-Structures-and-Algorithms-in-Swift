...
extension Graph where Element: Hashable {
    ...
    func depthFirstSearch(from source: Vertex<Element>) -> [Vertex<Element>] {
        ...
        depthFirstSearch(vertex: source, pushed: &pushed, visited: &visited)

        return visited
    }

    private func depthFirstSearch(vertex: Vertex<Element>,
        ...
        for edge in edges(from: vertex) {
            ...
            if !pushed.contains(destination) {
                depthFirstSearch(vertex: destination, pushed: &pushed, visited: &visited)
            }
        }
    }
