...
public class Dijkstra<T: Hashable> {
    ...
    public func getAllShortestPath(from start: Vertex<T>) -> [Vertex<T> : [Edge<T>]] {
        var shortestPaths = [Vertex<T> : [Edge<T>]]()
        let pathsFromStart = shortestPath(from: start)

        for vertex in graph.vertices {
            let path = shortestPath(to: vertex, paths: pathsFromStart)
            shortestPaths[vertex] = path
        }

        shortestPaths[start] = nil
